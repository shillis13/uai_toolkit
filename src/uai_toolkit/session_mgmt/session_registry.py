#!/usr/bin/env python3
"""
session_registry.py — SQLite-backed session identity registry (renamed from uai_toolkit.session_mgmt.session_store.py).
...
class SessionStore:
    ...
    def session_is_live(self, tracking_id: str) -> bool:
        """Public liveness check: True iff this tracking_id's terminal is ACTUALLY
        live in tmux/zellij (ground-truth probe), never the stale stored `status`
        column (which lingers as 'running' after a session dies unreconciled).

        This is the signal the Git-Guardian commit path must consult before routing
        a request, so a dark committer can't silently swallow it (todo_0626). Returns
        False for an unknown tracking_id or a session with no terminal.
        """
        rec = self.get(tracking_id)
        if not rec:
            return False
        return self._session_is_live(rec)
    ...
    def _row_to_dict(self, row: sqlite3.Row, live_sessions: set[str] | None = None) -> dict:
        ...
        if _sdir and _tid:
            try:
                # UNION read (canonical accessor over legacy): the writers of these
                # keys (set_activity_state, write_start_history) route through the
                # state bridge, so enrolled/flipped sessions store them in the
                # canonical state.<uuid8>.json — a legacy-only read would go blind
                # once phase-2 moves every session onto canonical (todo_0495).
                _common = Path(AI_ROOT) / "ai_general" / "data" / "hooks" / "common"
                if str(_common) not in sys.path:
                    sys.path.insert(0, str(_common))
                from uai_toolkit.hooks.common.lib_session_state_union import read_union
                _st = read_union(_sdir, _tid)
